window.document.addEventListener("DOMContentLoaded", () => {
	...
	if (video !== null) {
		if (video.duration > video.currentTime) {
			durationtimeVid.textContent = formatTimeVid(video.duration);
		}

		video.addEventListener("timeupdate", () => {
			// decrease duration.
			const time = video.duration - video.currentTime;
			durationtimeVid.textContent = formatTimeVid(time);
		});

		video.addEventListener("timeupdate", () => {
			// increase current time
			currenttimeVid.textContent = formatTimeVid(video.currentTime);
		});
	}
	// volume =============================//
